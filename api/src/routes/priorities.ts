...
import { priorities } from "../db/schema.js";
...
export default async function priorityRoutes(app: FastifyInstance) {
  ...
  app.get("/api/projects/:projectId/priorities", async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    const list = await db
      .select()
      .from(priorities)
      .where(or(eq(priorities.projectId, parsed.data.projectId), isNull(priorities.projectId)));
    return reply.send(list.sort((a, b) => a.sortOrder - b.sortOrder));
  });

  app.post("/api/projects/:projectId/priorities", async (req: FastifyRequest, reply: FastifyReply) => {
    const payload = req.user as { sub: string } | undefined;
    if (!payload) return replyError(reply, 401, "Unauthorized", "UNAUTHORIZED");
  ...
  app.get("/api/priorities/:id", async (req: FastifyRequest, reply: FastifyReply) => {
    const payload = req.user as { sub: string } | undefined;
    if (!payload) return replyError(reply, 401, "Unauthorized", "UNAUTHORIZED");
    ...
    const db = await getDb();
    const [row] = await db.select().from(priorities).where(eq(priorities.id, paramsResult.data.id)).limit(1);
    if (!row) return replyError(reply, 404, "Priority not found", "NOT_FOUND");
    if (row.projectId && !(await assertProjectAccess(db, row.projectId, payload.sub))) {
      return replyError(reply, 404, "Priority not found", "NOT_FOUND");
    }
    return reply.send(row);
  });

  app.patch("/api/priorities/:id", async (req: FastifyRequest, reply: FastifyReply) => {
    const payload = req.user as { sub: string } | undefined;
    if (!payload) return replyError(reply, 401, "Unauthorized", "UNAUTHORIZED");
    const paramsResult = paramsId.safeParse((req as FastifyRequest<{ Params: unknown }>).params);
    if (!paramsResult.success) return replyError(reply, 400, "Invalid id", "VALIDATION_ERROR");
    ...
    if (!bodyResult.success) return replyError(reply, 400, bodyResult.error.message, "VALIDATION_ERROR");
    const db = await getDb();
    const [existing] = await db.select().from(priorities).where(eq(priorities.id, paramsResult.data.id)).limit(1);
    if (!existing) return replyError(reply, 404, "Priority not found", "NOT_FOUND");
    ...
    const [updated] = await db
      .update(priorities)
      .set(setPayload)
      .where(eq(priorities.id, paramsResult.data.id))
      .returning();
    return reply.send(updated);
  });

  app.delete("/api/priorities/:id", async (req: FastifyRequest, reply: FastifyReply) => {
    const payload = req.user as { sub: string } | undefined;
    if (!payload) return replyError(reply, 401, "Unauthorized", "UNAUTHORIZED");
    const paramsResult = paramsId.safeParse((req as FastifyRequest<{ Params: unknown }>).params);
    if (!paramsResult.success) return replyError(reply, 400, "Invalid id", "VALIDATION_ERROR");
    ...
    await db.delete(priorities).where(eq(priorities.id, paramsResult.data.id));

...
import { shareTokens, testPlans, milestones, runs, tests, results } from "../db/schema.js";
import { eq, inArray } from "drizzle-orm";
import { replyError } from "../lib/errors.js";
import { assertProjectAccess } from "../lib/projectAccess.js";

const paramsToken = z.object({ token: z.string().min(1) });
const paramsPlanId = z.object({ id: z.string().uuid() });
...
export default async function shareRoutes(app: FastifyInstance) {
  app.get("/api/shares/:token", async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    if (row.resourceType === "plan") {
      const [plan] = await db.select().from(testPlans).where(eq(testPlans.id, row.resourceId)).limit(1);
      if (!plan) return replyError(reply, 404, "Plan not found", "NOT_FOUND");
      const planRuns = await db.select().from(runs).where(eq(runs.planId, plan.id));
  ...
  app.post("/api/plans/:id/share", { preValidation: [app.authenticate] }, async (req: FastifyRequest, reply: FastifyReply) => {
    const payload = req.user as { sub: string } | undefined;
    if (!payload) return replyError(reply, 401, "Unauthorized", "UNAUTHORIZED");
    const parsed = paramsPlanId.safeParse((req as FastifyRequest<{ Params: unknown }>).params);

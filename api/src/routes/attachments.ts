...
async function assertCaseProjectAccess(db: ReturnType<Awaited<ReturnType<typeof getDb>> extends infer D ? () => D : never> extends () => infer R ? R : never, caseId: string, userId: string): Promise<boolean> {
...
export default async function attachmentRoutes(app: FastifyInstance) {
  ...
  app.post("/api/cases/:id/attachments", async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    const exists = await assertCaseProjectAccess(db, params.data.id, payload.sub);
    if (!exists) return replyError(reply, 404, "Case not found", "NOT_FOUND");
    ...
    const key = `case/${params.data.id}/${crypto.randomUUID()}-${file.filename}`;
  ...
  app.post("/api/results/:id/attachments", async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    const key = `result/${params.data.id}/${crypto.randomUUID()}-${file.filename}`;
    await uploadFile(key, body, file.mimetype);
